...
def reveal_message(image_path):
    ...
    try:
        ...
        print(f"오류: '{image_path}' 파일을 찾을 수 없음. 파일 경로를 확인 필요.")
        return ""
    ...
    if img.mode not in ['RGB', 'RGBA']:
        print(f"경고: 이미지 모드가 {img.mode}임. RGB로 변환하여 처리.")
        img = img.convert('RGB')
    ...
    print("경고: 이미지에서 종료 마커 '###END###'를 찾지 못함.")
...
if __name__ == "__main__":
    ...
    if extracted_text:
        ...
        print("메시지 추출에 실패했거나 숨겨진 메시지가 없음.")